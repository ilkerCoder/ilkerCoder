(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const l of document.querySelectorAll('link[rel="modulepreload"]'))s(l);new MutationObserver(l=>{for(const t of l)if(t.type==="childList")for(const i of t.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function n(l){const t={};return l.integrity&&(t.integrity=l.integrity),l.referrerPolicy&&(t.referrerPolicy=l.referrerPolicy),l.crossOrigin==="use-credentials"?t.credentials="include":l.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function s(l){if(l.ep)return;l.ep=!0;const t=n(l);fetch(l.href,t)}})();class y extends HTMLElement{updateActiveNavigation(){this.querySelectorAll(".main-navigation__link").forEach(t=>{t.classList.remove("main-navigation__link--active")});const n=document.querySelector(".main-content"),s=n?.querySelector("under-construction");if(n?.querySelector("#avatar-sec")&&!s){const t=this.querySelector('.main-navigation__link[data-path="/about"]');t&&t.classList.add("main-navigation__link--active")}else if(s){const t=sessionStorage.getItem("lastClickedPath");if(t){const i=this.querySelector(`.main-navigation__link[data-path="${t}"]`);i&&i.classList.add("main-navigation__link--active")}}}connectedCallback(){this.innerHTML=`<header class="main-header is--container">
...
<nav class="main-navigation">
  <ul class="main-navigation__list is--nolist is--nomargin">
    ...
    `}}customElements.define("under-construction",f);const k=()=>{const o=document.querySelectorAll("svg g polygon");o.length!==0&&setInterval(()=>{const e=[],n=new Set;for(;e.length<8&&n.size<o.length;){const s=Math.floor(Math.random()*o.length);n.has(s)||(n.add(s),e.push(o[s]))}e.forEach((s,l)=>{setTimeout(()=>{s.classList.add("force-hover"),setTimeout(()=>{s.classList.remove("force-hover")},1e3)},l*200)})},1e3)};k();document.querySelectorAll("polygon").forEach(o=>{o.addEventListener("mouseenter",()=>{o.classList.add("animate"),o.addEventListener("animationend",()=>{o.classList.remove("animate")},{once:!0})}),o.addEventListener("touchstart",()=>{o.classList.add("animate"),o.addEventListener("animationend",()=>{o.classList.remove("animate")},{once:!0})})});function a(){const o=document.querySelector(".main-content");document.querySelectorAll(".main-navigation__link").forEach(l=>{l.addEventListener("click",t=>{t.preventDefault();const i=l.getAttribute("href");i==="/"||i==="/about"?n():s()})});function n(){o&&(o.innerHTML=`
        <div id="avatar-sec">
          <me-avatar></me-avatar>
      ...
      `,setTimeout(()=>{const l=document.getElementById("vertical-typewriter");l&&p(l)},100))}function s(){o&&(o.innerHTML="<under-construction></under-construction>")}}function p(o){const e=o.dataset.typetext||"";let n=-1,s=!0;o.innerHTML="",o.classList.add("show-cursor"),setInterval(()=>{s&&(n<e.length-1?(n++,o.innerHTML+=e.charAt(n)):(s=!1,setTimeout(()=>{const l=setInterval(()=>{o.innerHTML.length>0?o.innerHTML=o.innerHTML.slice(0,-1):(clearInterval(l),n=-1,s=!0)},50)},1500)))},120)}document.addEventListener("DOMContentLoaded",()=>{a(),c(),setTimeout(()=>{const o=document.getElementById("vertical-typewriter");o&&p(o)},250)});function c(){document.querySelectorAll(".main-navigation__link").forEach(e=>{e.addEventListener("touchstart",()=>{e.classList.add("touched")}),e.addEventListener("touchend",()=>{setTimeout(()=>{e.classList.remove("touched")},300)})})}
